...
Page({
  ...
  getPostList: function() {
    ...
    request({
      url: url+'?page='+this.data.page+'&pageSize='+this.data.pageSize,
      method: 'POST',
      header: {
        'content-type': 'application/json' // 默认值
    },
      realSuccess: function (data) {
        var postListItem = that.data.postList;
        var postList = data.list;
        if(postList.length<that.data.pageSize){
          that.setData({
            postList:postListItem.concat(postList),
            hasMore:false
          })
        }else{
          that.setData({
